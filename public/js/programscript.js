...
$(document).ready(() => {
    ...
    window.onload = function() {
        let relaxing = relaxingArray[Math.floor(Math.random() * relaxingArray.length)];
        console.log(relaxing)
    
        let strenuous = strenuousArray[Math.floor(Math.random() * strenuousArray.length)];
        console.log(strenuous);
        let insane = insaneArray[Math.floor(Math.random() * insaneArray.length)];
        let surprise = surpriseArray[Math.floor(Math.random() * surpriseArray.length)];
