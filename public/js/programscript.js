...
const insaneArray = ['CHALLENGING', 'MENTAL', 'BALLS TO THE WALL', 'ARNOLD SCHWARZENNEGER', 'OFF THE CHARTS', 'OFF MY TROLLEY', 'NUTS', 'BADASS', 'INSANE', 'GYM RAT', 'SHITS & GIGGLES', 'STORMIN NORMIN', 'ON A RAMPAGE', 'WHO LET THE DOGS OUT', 'SCREAMIN AB-DABS', 'BAT SHIT CRAZY', 'TASMANIAN DEVIL', 'BRING IT ON', 'RAT UP A DRAINPIPE', 'SKULL CRUSHING', 'ONE TOKE OVER THE LINE', 'PEDAL TO THE METAL'];

const surpriseArray = ['SURPRISE ME', 'INTRIGUE ME', 'OH BEHAVE!'];

window.onload = function() {
    const relaxing = relaxingArray[Math.floor(Math.random() * relaxingArray.length)];
    console.log(relaxing)

    const strenuous = strenuousArray[Math.floor(Math.random() * strenuousArray.length)];
    console.log(strenuous);
    const insane = insaneArray[Math.floor(Math.random() * insaneArray.length)];
    const surprise = surpriseArray[Math.floor(Math.random() * surpriseArray.length)];
